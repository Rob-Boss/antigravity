<script lang="ts">
    ...
    export let interactionRadius = 80;
    export let restoreSpeed = 0.02;
    export let volatility = 0.5;
    ...
    function update(dt: number) {
        ...
        // Adjust Physics Constants by time
        const timeDamping = Math.pow(DAMPING, dt);
        ...
        particles.forEach((p) => {
            ...
            if (distance < interactionRadius) {
                ...
                // Displacement-based impulses normalized by dt
                p.vx += (mouseVx * force * 0.05) / dt;
                p.vy += (mouseVy * force * 0.05) / dt;

                p.vx += Math.cos(angle) * force * MOUSE_FORCE * dt;
                p.vy += Math.sin(angle) * force * MOUSE_FORCE * dt;
            }

            const homeDx = p.originX - p.x;
            const homeDy = p.originY - p.y;
            p.vx += homeDx * restoreSpeed * dt;
            p.vy += homeDy * restoreSpeed * dt;

            if (volatility > 0) {
                p.vx += (Math.random() - 0.5) * volatility * 0.1 * dt;
                p.vy += (Math.random() - 0.5) * volatility * 0.1 * dt;
            }
